      ...
      if (ecx_recvpkt(port, stacknumber))
      {
         ...
         if (ehp->etype == oshw_htons(ETH_P_ECAT))
         {
            ...
            if (idxf == idx)
            {
               ...
               rt_memcpy(rxbuf, &(*stack->tempbuf)[ETH_HEADERSIZE], (*stack->txbuflength)[idx] - ETH_HEADERSIZE);
               /* return WKC */
               rval = ((*rxbuf)[l] + ((uint16)((*rxbuf)[l + 1]) << 8));
               /* mark as completed */
               (*stack->rxbufstat)[idx] = EC_BUF_COMPLETE;
            ...
            else
            {
               ...
               if (idxf < EC_MAXBUF && (*stack->rxbufstat)[idxf] == EC_BUF_TX)
               {
                  ...
                  rt_memcpy(rxbuf, &(*stack->tempbuf)[ETH_HEADERSIZE], (*stack->txbuflength)[idxf] - ETH_HEADERSIZE);
                  /* mark as received */
...
static int ecx_waitinframe_red(ecx_portt *port, uint8 idx, osal_timert timer)
{
   ...
   if (port->redstate != ECT_RED_NONE)
   {
      ...
      if ( ((primrx == RX_SEC) && (secrx == RX_PRIM)) )
      {
         ...
         rt_memcpy(&(port->rxbuf[idx]), &(port->redport->rxbuf[idx]), port->txbuflength[idx] - ETH_HEADERSIZE);
         wkc = wkc2;
      }
           ...
           ((primrx == RX_PRIM) && (secrx == RX_SEC)) )
      {
         ...
         if ( (primrx == RX_PRIM) && (secrx == RX_SEC) )
         {
            ...
            rt_memcpy(&(port->txbuf[idx][ETH_HEADERSIZE]), &(port->rxbuf[idx]), port->txbuflength[idx] - ETH_HEADERSIZE);
         }
